defmodule Stealth.HKDF do
  ...
  keys are derived in two steps: 
    ...
    1. Extract - a pseudorandom key is extracted from an input key masterial
                and optional salt. 
  ...
  @type slat :: binary
  @type pseudorandom :: binary
  @type lgnth :: non_neg_integer
  @type info :: binary
  ...
  """
  def derive(hash_func, ikm, len, salt \\ "", info \\ "") do
    prk = extract(hash_func, ikm, salt)
    expand(hash_func, prk, len, info)
  end
  ...
  """
  def extract(hash_func, ikm, salt \\ "") do
    :crypto.mac(:hmac, hash_func, salt, ikm)
  end
  ...
  ## Example
      ...
      <<28, 213, 201, 204, 16, 226, 160, 120, 69, 47, 46, 58, 15, 255, 54, 52>>
  """
  def expand(hash_func, prk, len, info \\ "") do
    hash_len = hash_length(hash_func)
    n = Float.ceil(len / hash_len) |> round()

    full =
      Enum.scan(1..n, "", fn index, prev ->
        data = prev <> info <> <<index>>
        :crypto.mac(:hmac, hash_func, prk, data)
      end)
